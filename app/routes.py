...
#CODIGO F401 'flask.abort' y 'flask.current_app' fueron importados pero no se usaron
...
def products():
    ...
    if request.method == 'POST':
        ...
        p = Product(name=request.get_json()
        ['name'], price=request.get_json()['price'])
        db.session.add(p)
        db.session.commit()
...
def addProductToOrder(pk):
    ...
    if (product_exists):
        return jsonify({'error': '<product {}> exists in <order {}>. Use PUT method'.format(product['id'], pk)}), 400
